...
| Guideline 4.2.2 remediation: personalized topics | Implemented — not yet Mac-verified | `Topic`, `TopicPreferencesStore`, `TopicArticleCache`; topic bar + `TopicsViewController`; `NewsListViewModel` topic state kept separate from the general feed; `NewsAPIService` gained an optional `category` query parameter with no change to its default (untopic'd) request shape. See README "Personalization, breaking-news alerts, and read-aloud". |
| Guideline 4.2.2 remediation: breaking-news alerts | Implemented — not yet Mac-verified | `NotificationScheduler` (local `UNUserNotificationCenter`, no push server) and `BreakingNewsNotifier`, hooked into the one path both a foreground refresh and `BackgroundRefreshManager` already share (`DefaultNewsRepository.fetchHeadlines(page: 1, ...)`); opt-in from the Topics screen; a tap routes through the existing, already-tested `DeepLinkRouter`. |
| Guideline 4.2.2 remediation: listen to articles | Implemented — not yet Mac-verified | `ArticleSpeechController` (`AVSpeechSynthesizer`) wired to a Listen/Stop control on `ArticleDetailView`; stops automatically on screen disappear. |
| Guideline 4.2.2 remediation: Home Screen widget | Source written; Xcode target not yet created | `NewslyWidget/` at the repo root (`TimelineProvider`, entry view, widget/bundle) plus `WidgetDataWriter` in the app target, sharing data through an App Group that safely no-ops until configured. `docs/WIDGET_SETUP.md` has the exact manual Xcode steps this specifically requires. |
